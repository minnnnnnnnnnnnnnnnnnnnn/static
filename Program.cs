...
class Program
{
	...
	private static string Head( string pageTitle ) 
	{
		...
		<meta charset="utf-8" />
		<base href="/static/" />
		<title>{{ ( string.IsNullOrEmpty( pageTitle ) ? "" :$"{ pageTitle } - " ) }}臺中市立臺中第一高級中等學校學生自治聯合會法規資料庫</title>
		...
		<link rel="apple-touch-icon" href="{{ File.ReadAllText( "./icon.txt" ) }}">
		<link rel="icon" href="{{ File.ReadAllText( "./icon.txt" ) }}">
		<link rel="bookmark" href="{{ File.ReadAllText( "./icon.txt" ) }}">
		<link rel="shortcut-icon" href="{{ File.ReadAllText( "./icon.txt" ) }}">
	...
	static void Main(string[] args)
	{
		...
		foreach( var la in las ) 
		{
			...
			using (StreamWriter o = File.AppendText($"./html/latest/{ la.No }.html")) 
			{
				...
				foreach( var lc in la.Content ) 
				{
					...
					o.WriteLine( "</dt>" ) ; 
					o.WriteLine( "<dd>" ) ; 
					o.WriteLine($"<a href=\"laws/law?a={ lc.LawURL[^10..] }&f=ln{ la.No }\">" ) ; 
		...
		using (StreamWriter o = File.AppendText("./html/cases/detail.html"))
		{
			...
			o.WriteLine( "qq.push( temp ) ; " ) ; 
			o.WriteLine( "} " ) ; 
			o.WriteLine( "const cases = qq.filter( i => i.case )[0] ; " ) ; 
			// o.WriteLine( "const no = qq.filter( i => i.no )[0] ; " ) ; 
			// o.WriteLine( "const no = qq.filter( i => i.no )[0] ; " ) ; 
			o.WriteLine( "if( cases ) " ) ; 
			o.WriteLine( "{" ) ; 
			o.WriteLine( "window.location.replace( document.baseURI + \"cases/detail/\" + cases.case ) ; " ) ; 
			o.WriteLine( "}" ) ; 
			o.WriteLine( "</script>" ) ; 
			o.WriteLine( foot ) ; 
		}
